...
def main(args):
    ...
    disp, kpts_fix, kpts_mov_corr = corrfield(img_fix, mask_fix, img_mov, alpha, beta, gamma, delta, lambd, sigma, sigma1, L, N, Q, R, T)
    
    torch.cuda.synchronize()
    t1 = time.time()
    
    np.savetxt('{}.csv'.format(output_path), torch.cat([kpts_fix[0], kpts_mov_corr[0]], dim=1).cpu().numpy(), delimiter=",", fmt='%.3f')
    nib.save(nib.Nifti1Image(disp[0].cpu().numpy(), np.eye(4)), '{}.nii.gz'.format(output_path))
